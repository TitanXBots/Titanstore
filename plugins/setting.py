...
async def settings_cb(client: Client, query: CallbackQuery):
    ...
    if data == "add_channels_menu":
    ...
    elif data in ["protect_menu", "protect_on", "protect_off"]:
        if data == "protect_on": await set_protect_status(True)
        elif data == "protect_off": await set_protect_status(False)
        is_on = await get_protect_status()
        status = "ᴏɴ ✅" if is_on else "ᴏꜰꜰ ❌"
        text = f"🔒 <b>ᴘʀᴏᴛᴇᴄᴛ ᴄᴏɴᴛᴇɴᴛ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ</b>\n\nᴘʀᴇᴠᴇɴᴛꜱ ᴜꜱᴇʀꜱ ꜰʀᴏᴍ ꜰᴏʀᴡᴀʀᴅɪɴɢ ᴏʀ ꜱᴀᴠɪɴɢ.\n\nᴄᴜʀʀᴇɴᴛ ꜱᴛᴀᴛᴜꜱ: <b>{status}</b>"
        ...
        text = f"📁 <b>ᴅᴀᴛᴀʙᴀꜱᴇ ᴄʜᴀɴɴᴇʟ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ</b>\n\nᴄᴜʀʀᴇɴᴛ ᴅʙ ᴄʜᴀɴɴᴇʟ ɪᴅ: <code>{current_db}</code>"
        return await safe_edit(query.message, text, InlineKeyboardMarkup([
        ...
        except ListenerTimeout: return await safe_edit(query.message, "📁 <b>ᴅᴀᴛᴀʙᴀꜱᴇ ᴄʜᴀɴɴᴇʟ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ</b>", back_keyboard)

        text_input = input_msg.text or ""
        try: await input_msg.delete()
        except: pass
        if not text_input or text_input.lower() == "/cancel": return await safe_edit(query.message, "❌ <b>ᴄᴀɴᴄᴇʟʟᴇᴅ!</b>", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 ʙᴀᴄᴋ", callback_data="add_channels_menu")]]))

        try:
            new_id = int(text_input)
            if not str(new_id).startswith("-100"): raise ValueError
        except: return await safe_edit(query.message, "❌ <b>ɪɴᴠᴀʟɪᴅ ɪᴅ!</b>\n\nᴍᴜꜱᴛ ꜱᴛᴀʀᴛ ᴡɪᴛʜ -100.", back_keyboard)

        await set_global_db_channel(new_id)
        current_db = await get_global_db_channel()
        text = f"✅ <b>ᴅʙ ᴄʜᴀɴɴᴇʟ ᴜᴘᴅᴀᴛᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ!</b>\n\nᴄᴜʀʀᴇɴᴛ ᴅʙ ᴄʜᴀɴɴᴇʟ ɪᴅ: <code>{current_db}</code>"
        return await safe_edit(query.message, text, InlineKeyboardMarkup([[InlineKeyboardButton("✏️ ᴄʜᴀɴɢᴇ ᴅʙ", callback_data="global_db_set"), InlineKeyboardButton("🔙 ʙᴀᴄᴋ", callback_data="add_channels_menu")]]))

    elif data in ["global_fs_menu", "forcesub_on", "forcesub_off"]:
        if data == "forcesub_on": await set_force_sub_status(True)
        elif data == "forcesub_off": await set_force_sub_status(False)
        ...
        text = f"📢 <b>ꜰᴏʀᴄᴇ ꜱᴜʙ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ</b>\n\nꜱᴛᴀᴛᴜꜱ: <b>{status}</b>\n\nᴄᴜʀʀᴇɴᴛ ᴄʜᴀɴɴᴇʟꜱ:\n{fs_str}"
        return await safe_edit(query.message, text, InlineKeyboardMarkup([
        ...
        await safe_edit(query.message, "📢 <b>ꜱᴇᴛ ꜰᴏʀᴄᴇ ꜱᴜʙ ᴄʜᴀɴɴᴇʟꜱ</b>\n\nꜱᴇɴᴅ ɪᴅꜱ (ꜱᴘᴀᴄᴇ ꜱᴇᴘᴀʀᴀᴛᴇᴅ, ᴏʀ 0 ᴛᴏ ᴄʟᴇᴀʀ)\n\n/cancel - ᴄᴀɴᴄᴇʟ.", back_keyboard)
        try: input_msg = await client.listen(chat_id=query.message.chat.id, timeout=60)
        except ListenerTimeout: return await safe_edit(query.message, "📢 <b>ꜰᴏʀᴄᴇ ꜱᴜʙ ᴄʜᴀɴɴᴇʟꜱ</b>", back_keyboard)

        text_input = input_msg.text or ""
        try: await input_msg.delete()
        except: pass
        if not text_input or text_input.lower() == "/cancel": return await safe_edit(query.message, "❌ <b>ᴄᴀɴᴄᴇʟʟᴇᴅ!</b>", back_keyboard)
        ...
        return await query.answer("✅ FS CHANNELS UPDATED!", show_alert=True)
