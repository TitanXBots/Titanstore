...
async def link_generator(client: Client, message: Message):
    ...
    if cmd == "genlink":
        ...
        try: r_msg = await client.listen(chat_id=message.chat.id, timeout=60)
        ...
        if not msg_id: return await r_msg.reply_text("❌ <b>ɪɴᴠᴀʟɪᴅ ᴍᴇꜱꜱᴀɢᴇ!</b> ᴍᴜꜱᴛ ʙᴇ ꜰᴏʀᴡᴀʀᴅᴇᴅ ꜰʀᴏᴍ ʏᴏᴜʀ ᴅʙ ᴄʜᴀɴɴᴇʟ.")
        ...
        try: first_msg = await client.listen(chat_id=message.chat.id, timeout=60)
        ...
        try: second_msg = await client.listen(chat_id=message.chat.id, timeout=60)
        except ListenerTimeout: return await message.reply_text("⌛ <b>ᴛɪᴍᴇᴏᴜᴛ!</b>")
